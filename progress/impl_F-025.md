...
| **Mutantes generados y supervivientes** | **Ejecutada** —`python -m harness.mutacion --feature F-025 --base feature/F-012-grafico-sigrid --workers 1`, 2026-09-11 13:29, informe en `progress/mutacion_F-025.md`—: **0 mutantes generados, 0 evaluados, 0 muertos, 0 supervivientes, 0 timeouts, 0,0 s, 1 worker**. El cero es **N/A justificado por el lenguaje del entregable**, no una puerta superada: las 0 líneas de alcance salen de que los cinco `.py` del diff viven bajo `tests/`, que `harness/alcance.py::es_produccion` excluye por diseño, y de que F-025 **no cambia ni una línea de Python de producción** (el entregable es JavaScript, HTML, tests y documentación, y `harness.mutacion` solo muta Python). Lo que sustituye a esa puerta está en §30.3: las mutaciones a mano sobre `js/pipeline.js`. *(Fila corregida tras la review: decía «No ejecutada» y la campaña sí se había lanzado — reparo 3 de su §7.)* |

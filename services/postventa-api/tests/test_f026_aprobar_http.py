...
# --------------------------------------------------------------------------
# RETIRADOS POR F-028 (T14, 2026-09-16) · el bloque `aprobacion` de `/api/parte`
# --------------------------------------------------------------------------
#
# Eran cinco, y probaban **el bloque de respuesta que T14 sustituye**, no un
# comportamiento que siga existiendo: `POST /api/parte` ya no devuelve
# `aprobacion` —una aprobación suelta— sino `estado`, que sale de los tres
# hechos a la vez (`design.md` §5). Los cuatro primeros se ponían **rojos** con
# T14 con un `KeyError: 'aprobacion'`; no se han «adaptado» cambiándoles la
# clave, que es lo que los habría dejado verdes probando otra cosa.
#
# | Retirado | Qué probaba | Sustituto en F-028 |
# |---|---|---|
# | `test_f026_r22_guardar_un_parte_aprobado_lo_dice_en_la_respuesta` | Que la respuesta cuenta lo que una persona aprobó, sin una petición por parte | `test_f028_r39_un_parte_que_aprobo_una_persona_lo_dice_en_la_respuesta` |
...
# | `test_f026_r22_una_aprobacion_revocada_se_devuelve_como_revocada` | «Se decidió y dejó de valer» ≠ «nadie decidió» (R31) | `test_f028_r19_una_aprobacion_sobre_otro_veredicto_no_firma_el_estado` |
...
# | `test_f026_r38_la_respuesta_de_guardar_tampoco_publica_el_oid` | Que esa respuesta no lleva el `oid` de quien decidió | `test_f028_r42_la_respuesta_de_guardar_tampoco_publica_el_oid_ni_el_motivo` |
#
# El quinto **seguía en verde** y por eso merece su párrafo: comprobaba que no
# se filtraba el `oid` de un bloque `aprobacion` que ya no se emite, así que a
# partir de T14 era verdad por vacío. Su sustituto comprueba lo mismo sobre el
# bloque que sí viaja, y además que no sale **el motivo** (R42, R52), que es lo
# que más peligro tiene porque lo escribe una persona en texto libre.
#
# Con ellos se van `_guardar`, `_aprobacion_guardada` y la anotación de
# `consultar_aprobacion` en `RepositorioQueAnotaElOrden`: no los usaba nadie
# más, y un ayudante sin llamantes es código muerto que el día de T15 alguien
# tendría que volver a leer para borrarlo.
#
# **Lo que NO se va de aquí**: todo lo de `POST /api/aprobar`, que sigue vivo
# hasta T15 y sigue devolviendo su bloque `aprobacion`.
